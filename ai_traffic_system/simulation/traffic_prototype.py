...
class Car:
    ...
    def move(self):
        # Define the "intersection point" — once car crosses this, let it move freely
...
def draw_background():
    ...
    # --- Center Dotted Divider per Lane (Only ONE per direction, clipped outside intersection) ---

    mid_x = WIDTH // 2
    mid_y = HEIGHT // 2
    gap = LANE_WIDTH // 2 + 10

    # Vertical (Lane 2 and 4), stop at intersection bounds
    for y in range(0, mid_y - gap, 40):
        pygame.draw.line(SCREEN, WHITE, (mid_x, y), (mid_x, y + 20), 2)
    for y in range(mid_y + gap, HEIGHT, 40):
        pygame.draw.line(SCREEN, WHITE, (mid_x, y), (mid_x, y + 20), 2)

    # Horizontal (Lane 1 and 3), stop at intersection bounds
...
def draw_lights():
    pos = {
        # Moved further out from the intersection & to the right side of the outgoing stream
...
def run_simulation():
    ...
    while True:
        ...
        # Always draw cars, move only if simulation is running
        ...
        for lane in state.lane_queues:
            state.lane_queues[lane] = [
                car for car in state.lane_queues[lane]
                if 0 <= car.rect.x <= WIDTH and 0 <= car.rect.y <= HEIGHT
            ]
